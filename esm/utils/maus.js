 ...
 */
function initMove(elementQuery, moveCallback, mausRoot = "html") {
    const els = document.querySelectorAll(elementQuery);
    const root = document.querySelector(mausRoot);
    [...els].forEach((element) => {
        root.addEventListener("mousemove", (event) => moveCallback(root, element, event));
    });
}
function initBoundary(elementQuery, enterCallback = null, exitCallback = null) {
    const els = document.querySelectorAll(elementQuery);
    [...els].forEach((element) => {
